...
import { SavedRouteState, RouteListItem, ProcessedRoute, LoadedRoute } from "../types/route.types";
import { fileToProcessedPhoto, processedPhotoToFile } from "../../photo/utils/photoUtils";
...
interface RouteContextType {
  ...
  deleteSavedRoute: (persistentId: string) => Promise<void>;
...
export const RouteProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [currentLoadedPersistentId, setCurrentLoadedPersistentId] = useState<string | null>(null);
  const [hasUnsavedChanges, setHasUnsavedChanges] = useState(false);

  // Update route properties
  const updateRoute = useCallback((routeId: string, updates: Partial<ProcessedRoute>) => {
    setRoutes(prev => prev.map(route => {
