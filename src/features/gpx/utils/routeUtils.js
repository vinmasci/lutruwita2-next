...
export const getUnpavedPercentage = (route) => {
    try {
        if (!route || !route.unpavedSections)
            return 0;
        ...
        const unpavedDistance = route.unpavedSections.reduce((total, section) => {
...
export const getElevationGain = (route) => {
    try {
        if (!route)
            return 0;
        // Check direct statistics
