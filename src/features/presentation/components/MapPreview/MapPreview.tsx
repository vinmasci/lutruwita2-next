...
export const MapPreview: React.FC<MapPreviewProps> = ({ center, zoom, routes = [], className = '' }) => {
    ...
    map.current.once('load', () => {
      ...
      routes.forEach((route, index) => {
        ...
        if (route.geojson?.features?.[0]?.geometry?.coordinates) {
          route.geojson.features[0].geometry.coordinates.forEach((coord: [number, number]) => {
            bounds.extend(coord);
          });
        }
      });
