...
export const calculateUnpavedPercentage = withPerformanceTracking(
  (route) => {
...
export const RouteCard = ({ route }) => {
  console.time(`RouteCard-${route.id || 'unknown'}`);
  const navigate = useNavigate();
  const cardRef = useRef(null);
  
  // Limit the number of photos to improve performance
  const limitedPhotos = useMemo(() => {
    console.time(`limitPhotos-${route.id || 'unknown'}`);
    if (!route.photos || route.photos.length === 0) {
      console.timeEnd(`limitPhotos-${route.id || 'unknown'}`);
      return [];
    }
    const result = route.photos.slice(0, MAX_PHOTOS_PER_CARD);
    console.timeEnd(`limitPhotos-${route.id || 'unknown'}`);
    return result;
  }, [route.photos]);
  ...
  React.useEffect(() => {
    console.timeEnd(`RouteCard-${route.id || 'unknown'}`);
    return () => {
      // Cleanup
    };
  }, [route.id]);
...
export const RouteCardGrid = ({ routes }) => {
  return (
    <Grid container spacing={4}>
      {routes.map((route) => (
        <Grid item xs={12} sm={6} md={4} key={route.id || route.persistentId}>
