...
const getStartingLocation = async (geojson) => {
    try {
        ...
        if (firstFeature?.geometry?.type === 'LineString') {
            ...
            if (Array.isArray(firstCoordinate) && firstCoordinate.length >= 2) {
                ...
                const response = await fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${longitude},${latitude}.json?access_token=${MAPBOX_TOKEN}&types=place,region`);
                if (!response.ok) {
