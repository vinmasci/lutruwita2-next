...
async function handleGetSavedRoutes(req, res) {
  try {
    ...
    // Get saved routes from Cloudinary
...
async function handleSaveRoute(req, res) {
  try {
    ...
    // Add route to user's saved routes in Cloudinary
...
async function handleRemoveRoute(req, res) {
  try {
    ...
    // Check if routeId is in the URL path (e.g., /api/user/saved-routes/route123)
    if (req.url.includes('/saved-routes/')) {
    ...
    // Remove route from user's saved routes in Cloudinary
...
const handler = async (req, res) => {
  ...
  console.log(`[API] Saved Routes - Request: ${req.method} ${req.url}`);
  console.log(`[API] Saved Routes - Headers:`, JSON.stringify(req.headers));
  console.log(`[API] Saved Routes - Body:`, JSON.stringify(req.body));
  
  // Parse the URL to extract path parameters
  const url = new URL(req.url, `http://${req.headers.host || 'localhost'}`);
  const pathParts = url.pathname.split('/').filter(part => part.length > 0);
  
  console.log(`[API] Saved Routes - Path parts:`, pathParts);
  
  // Extract routeId from path if present (for DELETE requests)
  if (req.method === 'DELETE' && pathParts.length > 0) {
    const lastPart = pathParts[pathParts.length - 1];
    if (lastPart && lastPart !== 'saved-routes') {
      // Add the routeId to the query params
      req.query.routeId = lastPart;
      console.log(`[API] Saved Routes - Extracted routeId from path: ${lastPart}`);
    }
  }
  ...
  switch (req.method) {
    ...
    case 'POST':
      console.log(`[API] Saved Routes - Handling POST request with body:`, JSON.stringify(req.body));
    ...
    default:
      console.log(`[API] Saved Routes - Method not allowed: ${req.method}`);
